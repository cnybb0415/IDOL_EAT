...
public class AdminIdolService {
	...
	public void write(HashMap<String, String> params, MultipartFile inputIdolImg, MultipartFile inputIdolMark) throws Exception {
		...
		logger.info("row : "+row);
		String idol_idx = dto.getIdol_idx();
	...
	public void upload(MultipartFile inputIdolImg, MultipartFile inputIdolMark, String idol_idx) throws Exception {
		//<1> 파일명 추출
		logger.info("업로드실행");
	...
	public void update(HashMap<String, String> params, 
		...
		dto.setIdol_group(params.get("idolAddGroup"));
		dto.setIdol_name(params.get("idolAddName"));
		logger.info("서비스 업데이트");
		logger.info("img : "+idol_img_ori);
		String idol_idx = params.get("idol_idx");
		String idol_group = params.get("idol_group");
		String idol_name = params.get("idol_name");
		logger.info("update ldx = "+idol_idx);
		int row = dao.update(idol_idx,idol_group,idol_name);
		logger.info("row : "+row);


		if(row > 0 && (!idol_img_ori.getOriginalFilename().equals("") && !idol_mark_ori.getOriginalFilename().equals(""))) {
			logger.info("음");
			upload(idol_img_ori, idol_mark_ori, idol_idx);
			try {
				Thread.sleep(1); //이름이 1ms를 기준으로 생성되어야만 겹치지 않으므로 일부러 sleep을 줬다.
			} catch (InterruptedException e) {
				e.printStackTrace();
	...
	public void idolBlindYes(String idol_idx) {
		dao.idolBlindYes(idol_idx);

	}

	public void idolBlindNo(String idol_idx) {

		dao.idolBlindNo(idol_idx);
